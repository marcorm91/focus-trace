...
The panel loads `devtools-panel.html` with that inspected tab id. The shared FocusTrace session hook recognizes the fixed id and pins Review, Structure, Trace and Report to the inspected page instead of following later browser-tab activation events.
...
No `chrome.debugger` permission is introduced.
...
The old in-sidepanel HTML/technical-selector inspector remains removed. Element location stays compact in 0.2.7; deeper browser-inspector affordances should build on DevTools-native capabilities rather than reproducing an HTML inspector inside FocusTrace cards.
...
7. Check Review, Structure, Trace and Report.
8. Confirm page-location actions affect the inspected page.
9. Close and reopen DevTools and confirm the panel reconnects to the current inspected tab session.
