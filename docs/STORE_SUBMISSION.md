...
Before uploading the production ZIP for 1.0.9:

1. Complete `npm run release:check:full` on the release candidate.
2. Confirm CI is green on the exact commit intended for `v1.0.9`.
...
9. Tag the exact approved commit as `v1.0.9` only after the release candidate is accepted.
