...
Before uploading the production ZIP for 0.2.3:

1. Complete `npm run release:check:full` on the release candidate.
2. Confirm CI is green on the exact commit intended for `v0.2.3`.
...
8. Tag the exact approved commit as `v0.2.3` only after the release candidate is accepted.
