...
This conservative model intentionally avoids converting uncertain rendering into false WCAG failures. Stacked backdrop verification handles opaque RGB and transparent RGBA separately and also treats separately positioned image/media elements (for example an absolute hero image behind white text) as painted backdrop evidence. The verification walks bounded ancestor-level stacking contexts instead of checking only the target's direct parent, inspects a bounded number of painted/media descendants inside sibling branches that can overlap the target, and treats generated `::before`/`::after` layers that are painted, positioned and full-inset as unresolved visual backdrops. Computed styles are cached for the verification pass; FocusTrace does not walk every authored stylesheet rule and run selector matching for each backdrop candidate. When such a layer can affect the target, FocusTrace removes the ancestor-derived ratio/background from the finding and downgrades it to REVIEW rather than reporting a deterministic failure against an unrelated ancestor color. It first samples the actual paint stack at the target center plus four interior points, then falls back to at most 8 ancestor levels, 12 sibling branches and 24 descendants per branch. It verifies at most 100 text/non-text contrast candidates and 800 computed-style observations per scan; exhaustion of either global budget or a local structural-search limit becomes REVIEW with an explicit unverified-background reason and adjusted rule counters.
...
`FT-WCAG-018` evaluates exposed `object[data]` elements through the shared accessible-name diagnostics. A non-empty name from supported ARIA or host-language naming sources, or explicit presentational treatment for genuinely decorative content, satisfies the bounded expectation. An exposed object with no usable alternative produces deterministic FAIL evidence for the implemented WCAG 1.1.1 subset; the rule does not claim every loading and fallback branch of the HTML object model.

`FT-WCAG-019` checks exposed `iframe` / `frame` elements for a non-empty accessible name using the same bounded name primitive. This includes supported `aria-labelledby`, `aria-label` and `title` evidence. The frame element itself is evaluated even when its nested browsing context cannot be inspected.

`FT-WCAG-020` applies only when a frame is removed from sequential navigation with a negative `tabindex` and its embedded document is locally inspectable under same-origin access. FocusTrace performs bounded descendant focusability inspection; an observed sequentially focusable descendant produces FAIL for that keyboard expectation. Persisted evidence contains only the frame target and, when relevant, the tag name of the observed focusable descendant. Text, attributes and arbitrary content from the embedded document are not copied into findings, Memory or reports. When the persisted **Ignore iframe contents** scan preference is enabled, nested-document traversal is disabled at the composed-tree source: the frame host remains eligible for frame-level naming/uniqueness checks, while descendant rules and `FT-WCAG-020` do not inspect the embedded document.

`FT-REVIEW-038` compares normalized accessible names of exposed frames. Repeated names remain REVIEW rather than automatic FAIL because identical names are valid when the embedded contexts have an equivalent purpose. `FT-REVIEW-039` records an explicit inspection boundary when a nested document is cross-origin, opaque, unavailable or sandboxed without `allow-same-origin`. Such a boundary is never interpreted as a clean nested-document result.

SVG and image-role alternatives reuse `FT-WCAG-002` rather than creating a second image finding. The extension recognizes direct SVG `<title>` text for supported image/graphics semantics while preserving the existing shared naming evidence. Active image-map areas continue to use the existing link-name rule `FT-WCAG-005`, including supported `alt`, ARIA and title naming paths.

The axe-core benchmark therefore classifies `frame-title`, `frame-title-unique`, `frame-focusable-content` and `object-alt` as partial FocusTrace coverage rather than equivalence. `area-alt`, `role-img-alt` and `svg-img-alt` reuse existing rule IDs with bounded partial coverage. `frame-tested` remains not applicable because it verifies axe orchestration/injection rather than an end-product accessibility requirement. Cross-origin or otherwise unavailable embedded content is reported as an evaluation boundary, not silently counted as tested.

## Per-finding Recheck methodology

Recheck is an evidence-revalidation workflow, not a second rule engine and not a mutation of the original finding. The first observation remains immutable. Each recheck appends a bounded record containing the current target resolution, current evidence where available, timestamp and one of five states: `resolved`, `persistent`, `changed`, `missing` or `inconclusive`.

Element identity is deliberately stricter than selector equality. FocusTrace stores a compact signature that can cross light DOM, open Shadow DOM and same-origin frame boundaries. A direct selector match is accepted only when the signature still agrees with the recorded target. If a selector now matches multiple candidates, the boundary cannot be traversed, or the signature materially disagrees, FocusTrace does not silently bind the old finding to a new node. The result remains `changed`, `missing` or `inconclusive` according to the evidence that can actually be established.

A static finding can become `resolved` only when the rule's `ScanRuleResult.coverage` is complete for the tested expectation and the current scan establishes a pass for that same rule/target context. Absence of a finding is insufficient when the rule is findings-only; in that case removal or non-reproduction stays `inconclusive`. If the same finding is emitted again for the resolved target, it is `persistent`. A target that can be resolved safely but no longer matches the original identity/evidence contract is `changed` rather than silently treated as the same observation.

Runtime findings are more conservative. Recheck may confirm whether the recorded target still resolves, but node presence alone cannot establish whether an interaction-dependent failure/review has been fixed. The runtime state therefore requires the relevant behavior to be exercised again with Trace; Recheck surfaces replay-required/inconclusive evidence rather than manufacturing `resolved` from a static lookup.

Recheck history updates the current session/report and the saved multipage audit. FocusTrace Memory can ingest the newer observation without discarding the original evidence and preserves an existing compact locator or visual preview for that remembered item. Historical audit pages are not probed in the background; a live Recheck is offered only when the relevant page is active. All identity resolution, rescanning and persistence remain local to the extension and use the existing page-access permissions.
...
## Guided keyboard, focus and dialog evidence

`FT-GUIDED-002`, `FT-GUIDED-003` and `FT-GUIDED-004` extend the guided/manual framework with keyboard operability, focus order/visibility and dialog lifecycle reviews. They reuse the existing runtime event stream rather than creating a second recorder. At save time, up to two relevant events from the current guided step are copied into bounded `runtime-observation` evidence; manual answers and auditor notes remain separate evidence kinds.
