...
`FT-WCAG-020` applies only when a frame is removed from sequential navigation with a negative `tabindex` and its embedded document is locally inspectable under same-origin access. FocusTrace performs bounded descendant focusability inspection; an observed sequentially focusable descendant produces FAIL for that keyboard expectation. Persisted evidence contains only the frame target and, when relevant, the tag name of the observed focusable descendant. Text, attributes and arbitrary content from the embedded document are not copied into findings, Memory or reports.
