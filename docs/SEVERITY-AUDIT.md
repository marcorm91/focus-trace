...
| `FT-REVIEW-017` Prerecorded audio alternative | serious | review | Missing observable local alternative evidence can make audio-only information unavailable, but FocusTrace cannot prove whether an equivalent transcript exists elsewhere or whether candidate text is actually equivalent to the recording. |
| `FT-REVIEW-018` Prerecorded captions | serious | review | Missing observable native captions can block access to auditory information. FocusTrace references ACT `f51b46` for the native-video captions expectation but keeps the result contextual because audio presence, burned-in captions and custom-player tracks are not always observable. |
| `FT-REVIEW-021` Prerecorded video alternative / audio description | serious | review | Missing observable description or media-alternative evidence can hide meaningful visual information. ACT `c5a4ea` informs the accessible-alternative expectation, but FocusTrace cannot prove that local transcript/details or description candidates are equivalent to the visual content. |
| `FT-REVIEW-022` Live captions | serious | review | Missing observable captions during strongly identified live synchronized video can block access to auditory information. The result remains contextual because custom-player or burned-in captions and actual audio applicability are not always observable. |
| `FT-REVIEW-023` Prerecorded audio description | serious | review | Missing observable audio-description evidence can hide meaningful visual information. ACT `1ec09b` informs the strict visual-alternative expectation, but custom described versions and the adequacy/applicability of descriptions require human judgement. |
| `FT-REVIEW-024` Reflow at narrow viewport | serious | review | Clipped content or two-dimensional scrolling can make ordinary content unavailable to users who enlarge it. The result remains contextual because essential two-dimensional layouts are permitted and FocusTrace observes only the current responsive state. |
| `FT-REVIEW-025` Inline link use of color | serious | review | Aligned with axe `link-in-text-block`. A link distinguished only by a small color difference can be unavailable to users who do not perceive that difference; FocusTrace keeps the outcome contextual because complete visual context and uncommon persistent cues still require human judgement. |
| `FT-REVIEW-026` Pause, stop or hide moving content | serious | review | Persistent movement can prevent users with cognitive, attention or visual disabilities from reading and operating the rest of the page. No direct axe equivalence is enforced; automatic start, essentiality and custom control behavior remain contextual. |
| `FT-REVIEW-027` Link purpose in context | serious | review | Generic link wording can hide a destination or action when programmatic context is insufficient. Proposed ACT `5effbb` informs applicability and context evidence, but natural-language purpose and the generally-ambiguous exception require human judgement; no direct axe equivalence is enforced. |
| `FT-REVIEW-028` Resize text at 200% | serious | review | Text or controls that disappear, become clipped/overlapped or fail to reach twice their effective size can remove information or functionality for people with low vision. The result remains contextual because responsive replacements, alternate resize mechanisms and complete functionality require human judgement. |
| `FT-REVIEW-019` Error identification | serious | review | Missing observable text association for an explicitly/user-invalid field can leave users without a usable error description. ACT `36b590` informs the error-message expectation, but FocusTrace keeps the result contextual because visual/application-level messages and semantic adequacy are not fully observable. |
| `FT-REVIEW-020` Error suggestion | moderate | review | A known constraint error may block recovery when correction guidance is not useful. FocusTrace restricts applicability to invalid fields with associated error text and correction-relevant constraint metadata, while suggestion quality and the security/purpose exception remain contextual. |
...
| `FT-REVIEW-015` Consistent identification across sampled pages | moderate | review | Exact unique native-link destinations provide a strong function anchor, but semantic functional equivalence and label meaning still require human context; no axe equivalent is enforced. |
