...
Current release candidate: **1.0.7**.
...
For the current candidate, the release version is **1.0.7** and the intended tag is **`v1.0.7`**.

- Confirm `package.json`, `package-lock.json` and all browser manifests report `1.0.7`.
- Confirm `tests/release-contract.test.ts` targets `v1.0.7` and passes.
- Confirm `docs/changelog/RELEASE_NOTES_1.0.7.md` and `docs/changelog/CHANGELOG.md` match the shipped behavior and limitations.
- Confirm the version shown in Settings comes from the installed manifest and displays `1.0.7` in the packaged candidate.
...
- Tag the exact approved commit as `v1.0.7`.
...
After publishing 1.0.7, update the candidate version at the top of this checklist when preparing the next release rather than copying a version-specific checklist.
