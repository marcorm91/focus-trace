...
| **Contraste de texto** | Texto renderizado con colores resolubles | Calcula ratio, umbral, foreground/background, tamaño y peso; los fondos pintados o de imagen en capas independientes, incluidos descendientes pintados dentro de ramas hermanas a distintos niveles de ancestros y pseudo-elementos pintados a tamaño completo, pasan a REVIEW cuando el fondo compuesto final no puede resolverse con seguridad. | Evidencia estructurada reutilizable en UI e informes sin presentar un color de ancestro como si fuera el fondo visual real. |
...
| **Estructura** | Análisis actual de página completa | Expone encabezados, revisión semántica y métricas estructurales preparadas con el análisis; Actualizar las recalcula tras cambios de la página. Los encabezados nativos que repiten de forma redundante role="heading"/aria-level se cuentan una sola vez, y los enlaces cuyo title solo repite su texto visible aparecen como recomendación de revisión. | Árbol H1-H6, sugerencias y recuentos; los encabezados sin texto directo pueden usar contenido accesible significativo de sus descendientes, mientras el contenido únicamente decorativo sigue considerándose vacío. |
| **Trace** | Interacción real | Registra teclado/puntero, foco, cambios no sensibles de controles, mutaciones relevantes, rutas SPA, diálogos, candidatos a mensajes de estado, widgets ARIA y evidencia de causalidad/cambios de contexto. Iniciar/reanudar Trace o Focus Walk comprueba el sitio de la pestaña seleccionada frente a los sitios de la auditoría activa antes de grabar. Una navegación de documento completo pausa Trace hasta validar la página nueva. | Eventos correlacionados por interacción y atribuidos a una URL de página redactada; el seguimiento de cambios de contexto no conserva valores de controles. |
...
| **Informe** | Evidencia estática/runtime disponible | Consolida análisis, historias runtime y Estructura ya generada. Las auditorías multipágina conservan hasta 200 eventos Trace recientes atribuidos por cada página revisada. | Vista de informe y exportaciones con evidencia local acotada. |
| **PDF / TXT / Markdown / JSON** | Informe actual o línea base de Memory | Exporta la evidencia y las notas del auditor aplicables reutilizando los datos disponibles sin volver a recorrer silenciosamente todo el DOM. Los PDF de auditoría completa sitúan la evidencia Trace conservada dentro de la sección de su página; los PDF individuales de página/componente incluyen un índice enlazado compacto con la tipografía normal, líneas de puntos y espaciado del índice multipágina, sin separadores sólidos entre filas. | Artefactos compartibles. |

Antes de iniciar Trace/Focus Walk (incluida la reanudación tras un breakpoint o una navegación de documento), otro sitio requiere elegir: cancelar, añadir el sitio a la auditoría actual o iniciar una independiente. Las páginas de un sitio ya aceptado no muestran el aviso. Una navegación completa durante la grabación pausa Trace, conserva la evidencia anterior y registra la transición origen/destino redactada; la grabación solo se reanuda cuando la página actual supera la misma validación de alcance. Cancelar conserva la traza anterior; detener una grabación nunca pide esta decisión. Navegar mientras se elige invalida la decisión. Elegir el alcance guarda solo la pertenencia a la auditoría, sin inventar un análisis ni una traza histórica. El modal adaptable se amplía hasta 720px, conservando márgenes laterales de 12px en paneles estrechos.

Cuando una página analizada pertenece a la auditoría multipágina activa, la evidencia Trace atribuida se guarda con esa página al detener, pausar por navegación, pausar por breakpoint o volver a analizarla. Las sincronizaciones repetidas deduplican los eventos por ID, y volver a analizar la misma URL actualiza su informe sin moverlo ni descartar el Trace guardado. Si las URL redactadas no permiten identificar una única página revisada, FocusTrace no adivina la asociación.

La cabecera de la auditoría actual y el modal de alcance muestran todos los dominios aceptados, con salto de línea en paneles estrechos; el nombre inicial ya no oculta que la auditoría abarca varios sitios.

**Informe → Más formatos → Exportar JUnit (.xml)** exporta los hallazgos de página/componente actuales o guardados con el mismo contrato JUnit de CLI/Playwright: solo FAIL genera un caso fallido. Los informes históricos exportan sus hallazgos estáticos sin reutilizar eventos de Trace actuales. Los botones Markdown/JSON exportan evidencia de Trace, no auditorías estáticas; cuando no están disponibles, el menú explica que falta la grabación.

Los hallazgos estáticos repetidos solo se deduplican cuando **regla, destino y evidencia coinciden**. Si cambia la evidencia para la misma regla/destino, el hallazgo sigue visible y pasa a `changed`; una identidad previa ausente queda como `resolved` únicamente en el historial comparativo y, si reaparece más adelante, vuelve a ser `new`. La comparación del ciclo de vida se limita a contextos compatibles de documento/alcance/perfil. Recheck mantiene además sus estados de seguridad `missing`/`inconclusive`, mientras Replay y Site Audit comparten `new`/`persistent`/`changed`/`resolved` cuando existe una comparación completa. Las coincidencias exactas de evidencia se reservan antes de emparejar los cambios, por lo que el orden de los resultados no convierte un hallazgo idéntico en cambiado.
...
Los perfiles de auditoría permanecen solo en el perfil del navegador y están limitados a un perfil completo integrado más **20 perfiles personalizados**. El perfil integrado conserva todos los hallazgos compatibles; los perfiles personalizados pueden seleccionar alcance de página/componente/Site Audit, todas las reglas compatibles o un objetivo WCAG A/AA/AAA, fuentes normativas, IDs exactos de regla FocusTrace, severidades y familias de reglas. Los perfiles filtran los hallazgos guardados/mostrados; no desactivan el motor local de reglas, no cambian el significado de PASS, no transmiten datos de la página ni modifican la página inspeccionada.
...
| `FT-REVIEW-028` | Una comparación guiada de la misma página con zoom del navegador al 100 % y al 200 % observa textos o controles que desaparecen, pierden su nombre, quedan recortados/solapados o no duplican su tamaño efectivo. | REVIEW | WCAG 1.4.4 AA |
...
| `FT-REVIEW-040` | Varias etiquetas nativas, nombre accesible solo mediante `title`, grupos de controles sin nombre, restricciones sin una relación de instrucciones resuelta o texto visible de obligatoriedad sin estado requerido programático. | REVIEW | WCAG 1.3.1 / 3.3.2 · Formularios HTML |
...
`FT-REVIEW-028` ofrece un flujo guiado de página completa para WCAG 1.4.4. Analiza con el zoom del navegador al 100 % para conservar durante la sesión una referencia de hasta 1.000 textos directos renderizados y controles interactivos; después ajusta el zoom al 200 % y vuelve a analizar el mismo documento. FocusTrace compara selectores estables y firmas acotadas de texto/nombre, descarta un original no disponible cuando observa un reemplazo renderizado equivalente y emite como máximo 30 revisiones por contenido que deja de estar disponible, nombres de control perdidos, recortes nuevos, solapamientos nuevos o una escala efectiva del texto inferior a 1,9:1. La evidencia JSON estructurada conserva ambos niveles de zoom, rectángulos de referencia/actuales, escala y selectores relacionados. FocusTrace nunca cambia el zoom. Los niveles intermedios, la equivalencia semántica, la interfaz del navegador/plataforma, las imágenes de texto, los subtítulos, transformaciones, texto generado/Shadow DOM, frames de otro origen y la funcionalidad completa siguen siendo manuales; una comparación sin candidatos no demuestra conformidad con WCAG 1.4.4.

Para `FT-REVIEW-019`, FocusTrace solo aplica cuando observa `aria-invalid` explícito distinto de `false` o `:user-invalid` compatible con el navegador. Un destino no vacío de `aria-errormessage` o `aria-describedby` cuenta únicamente como PASS acotado de presencia de asociación; FocusTrace no demuestra que el texto describa completamente el error ni que no exista otro mensaje visual o propio de la aplicación. `FT-REVIEW-020` solo se ejecuta cuando ese campo inválido ya tiene texto asociado y expone una restricción relevante para corregirlo, como `required`, un tipo de input restringido, `pattern`, `min`/`max`, `step` o límites de longitud. Sigue siendo REVIEW porque la suficiencia de la sugerencia y la excepción WCAG por seguridad/propósito requieren contexto. Ninguna de las dos reglas lee ni guarda el valor del campo, y el texto asociado del mensaje de error se usa únicamente para comprobar que existe contenido no vacío; el mensaje en sí no se persiste en la evidencia del hallazgo.

Para `FT-REVIEW-040`, FocusTrace agrupa señales acotadas de autoría de formularios que necesitan contexto humano: más de una `<label>` nativa asociada a un control, un control cuyo nombre accesible procede únicamente de `title`, un `fieldset`/`group`/`radiogroup` expuesto con varios controles pero sin nombre de grupo utilizable, metadatos de restricciones sin una relación `aria-describedby`/`aria-details` no vacía resuelta y texto visible de obligatoriedad sin `required` nativo ni `aria-required="true"`. Permanecen como **REVIEW** porque varias etiquetas o la ubicación de instrucciones pueden ser intencionadas y la suficiencia semántica no puede deducirse solo de la estructura. La regla nunca lee ni guarda valores editables de campos.
...
| Errores de formulario | La revisión estática observa estado inválido/`:user-invalid`, presencia de texto de error no vacío asociado mediante ARIA y metadatos de restricciones relevantes para la corrección. No lee valores de campos, no persiste el texto del mensaje asociado, no deduce todos los mensajes visuales o propios de la aplicación ni determina la suficiencia semántica de una sugerencia. |
| Etiquetas e instrucciones de formulario | `FT-REVIEW-040` inspecciona solo señales estructurales acotadas: varias etiquetas nativas, nombre solo mediante `title`, grupos sin nombre, restricciones sin relaciones de instrucciones resueltas y texto visible de obligatoriedad sin estado requerido programático. No juzga automáticamente la calidad de las instrucciones ni persiste valores editables. |
...
### Frames, SVG, mapas de imagen y contenido incrustado

FocusTrace amplía el modelo compartido de nombre accesible a las comprobaciones de contenido incrustado sin crear hallazgos paralelos para casos ya cubiertos por una regla existente. `FT-WCAG-018` comprueba que los `object[data]` expuestos tengan una alternativa accesible no vacía o un tratamiento presentacional explícito. `FT-WCAG-019` comprueba que los elementos `iframe` / `frame` expuestos tengan un nombre accesible no vacío. `FT-WCAG-020` comprueba frames excluidos de la navegación secuencial mediante `tabindex` negativo cuando su documento incrustado same-origin puede inspeccionarse localmente; observar un descendiente alcanzable mediante foco secuencial produce FAIL determinista para esa expectativa acotada de teclado.

`FT-REVIEW-038` mantiene como REVIEW los nombres normalizados de frame duplicados, porque dos nombres idénticos son válidos cuando los contextos incrustados tienen realmente un propósito equivalente. `FT-REVIEW-039` hace explícito el límite de inspección cuando el documento anidado no puede evaluarse por ser cross-origin, estar aislado por `sandbox` sin acceso same-origin, ser opaco o no estar disponible. Ajustes incorpora además la preferencia **Ignorar contenido de iframes**, desactivada por defecto. Al activarla, FocusTrace sigue evaluando el propio elemento `<iframe>` / `<frame>`, pero no recorre ni audita los descendientes de su documento incrustado, incluido contenido same-origin.

Las alternativas de SVG y roles de imagen siguen utilizando `FT-WCAG-002`, incluido el texto de un `<title>` SVG directo cuando aplica; las áreas activas de mapas de imagen siguen usando la regla existente de nombre de enlace `FT-WCAG-005`. Así se evitan hallazgos duplicados y se reutiliza la misma primitiva de nombre accesible. La inspección del documento incrustado solo conserva el límite de evaluación y, cuando es relevante, el nombre del tag de un descendiente con foco observado; no copia texto ni atributos del documento incrustado a hallazgos, Memory ni informes.

### Etiquetas, propósito, restricciones y errores de formulario

`FT-REVIEW-040` añade una familia REVIEW acotada de autoría de formularios para varias etiquetas nativas, nombres accesibles basados solo en `title`, grupos expuestos sin nombre, metadatos de restricciones sin una relación de instrucciones resuelta y texto visible de obligatoriedad sin un estado requerido programático. Las reglas existentes `FT-REVIEW-014`, `FT-REVIEW-019` y `FT-REVIEW-020` siguen cubriendo propósito de entrada, identificación de errores y evidencia de sugerencias de corrección.

La evidencia de formularios está limitada deliberadamente por privacidad. FocusTrace no lee ni persiste valores editables para estas comprobaciones y el texto asociado de mensajes de error ya no se copia a la evidencia del hallazgo: solo se conserva que existe texto no vacío y la relación usada. La calidad ambigua de instrucciones, si varias etiquetas son intencionadas y la suficiencia de la guía de corrección permanecen como REVIEW humano, no como FAIL automático.

### Viewport, zoom, reflow y presentación visual

`FT-WCAG-021` comprueba la sintaxis de viewport observable por ACT que puede bloquear la ampliación al 200 %: `user-scalable=no` o un `maximum-scale` finito y no negativo inferior a `2` producen un FAIL acotado. Esto no afirma conformidad completa con WCAG 1.4.4; los overrides del navegador y mecanismos alternativos de redimensionado quedan fuera de esta comprobación atómica de sintaxis.
...
La verificación de fondos sigue limitada a 100 candidatos de contraste por análisis y a 800 observaciones de estilo computado compartidas por esa pasada de verificación. Primero muestrea la pila de pintado real en el centro y cuatro puntos interiores del objetivo y, solo después, usa un fallback estructural mucho más pequeño (hasta 8 ancestros, 12 hermanos y 24 descendientes por rama). Reutiliza estilos y geometría ya calculados y no recorre todas las reglas/selectores de las hojas CSS para cada candidato. Si se agota cualquiera de los límites, los candidatos restantes quedan explícitamente como REVIEW porque su fondo no se ha verificado, con el motivo de agotamiento y contadores coherentes. Los colores RGB opacos (incluidos negro y rojo) se distinguen de los colores RGBA transparentes.
...
Informe reúne las pruebas guiadas en el acordeón **Comprobaciones manuales**, cerrado inicialmente. Revisión se centra en los hallazgos y deja de mostrar el resumen del ciclo de vida y la tarjeta de estado del zoom; se mantienen los datos de evolución y el análisis de ampliación.
...
Las sugerencias estructurales de Informe y del PDF de una página incluyen el elemento afectado, el texto o etiqueta disponible, el origen de la detección y un selector CSS copiable para localizar el hallazgo sin exportar el HTML completo.
...
La versión 1.0.9 compara la separación de objetivos de puntero dentro de su propio documento (incluidos los iframes del mismo origen) y reutiliza una medición de geometría/estilo por objetivo durante la evaluación. El recorrido compuesto deja de enumerar hermanos en cuanto se agota su límite compartido. La verificación de contraste usa las coordenadas reales fuera de pantalla en su comprobación estructural, omite puntos de muestreo fuera del viewport y marca como REVIEW los objetivos que no puede localizar. La evidencia en español distingue los límites de candidatos, lecturas de estilos y búsqueda local.

La revisión de sesiones y persistencia de la 1.0.9 también serializa las modificaciones de auditorías multipágina mediante un único escritor en segundo plano, evitando que varios paneles sobrescriban páginas guardadas. Los borrados esperan a las escrituras anteriores ya aceptadas, y el cierre de una pestaña elimina su sesión después de terminar las escrituras pendientes. El panel descarta lecturas antiguas de sesión/auditoría y respuestas de acciones de una selección anterior; cambiar de idioma ya no reinicia la selección de pestaña. Las decisiones de alcance pendientes se cancelan al cambiar de pestaña.
