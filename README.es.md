...
`FT-REVIEW-038` mantiene como REVIEW los nombres normalizados de frame duplicados, porque dos nombres idénticos son válidos cuando los contextos incrustados tienen realmente un propósito equivalente. `FT-REVIEW-039` hace explícito el límite de inspección cuando el documento anidado no puede evaluarse por ser cross-origin, estar aislado por `sandbox` sin acceso same-origin, ser opaco o no estar disponible. FocusTrace sigue evaluando el propio elemento frame y nunca trata un documento incrustado inaccesible como resultado limpio.
