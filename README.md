...
Before Trace/Focus Walk starts (including breakpoint resume), another site requires an explicit choice: cancel, add the site to the current audit, or start a separate audit. Pages on an already accepted site do not prompt. Cancelling preserves the previous Trace; stopping a recording never prompts. Navigation while choosing invalidates the decision. Choosing a scope saves audit membership only, not a fabricated scan or historical Trace snapshot. The responsive scope dialog expands up to 720px, keeping 12px side gutters in narrow panels.
