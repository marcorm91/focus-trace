...
| **Text contrast** | Rendered text with resolvable colors | Calculates ratio, required threshold, foreground/background, font size and weight; separately stacked painted/image backdrops are downgraded to REVIEW when the final composed background cannot be resolved safely. | Structured evidence reusable by UI and reports without reporting an ancestor color as the real image backdrop. |
