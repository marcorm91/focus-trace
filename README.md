...
For `FT-REVIEW-017` and `FT-REVIEW-018`, FocusTrace evaluates only bounded native-media evidence. Clear live/stream signals are skipped. Audio alternatives can be observed locally but their equivalence is not verified; caption tracks can be observed but burned-in/custom-player captions, auditory-content applicability and caption synchronization/accuracy/completeness remain manual. `FT-REVIEW-017` currently covers the native audio-only subset of WCAG 1.2.1, not its video-only branch.
