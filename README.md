...
Audit profiles are stored only in the browser profile and are bounded to one built-in complete profile plus **20 custom profiles**. The built-in profile preserves all supported findings; custom profiles can select page/component/Site Audit scope, all supported rules or a WCAG A/AA/AAA target, severities and FocusTrace rule families. Profiles filter stored/reporting findings; they do not disable the underlying local rule engine, alter PASS meaning, transmit page data or change the inspected page.
