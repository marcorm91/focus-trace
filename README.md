...
| **Text contrast** | Rendered text with resolvable colors | Calculates ratio, required threshold, foreground/background, font size and weight; separately stacked painted/image backdrops, including painted descendants inside ancestor-level sibling branches and full-inset painted pseudo-elements, are downgraded to REVIEW when the final composed background cannot be resolved safely. | Structured evidence reusable by UI and reports without reporting an ancestor color as the real visual backdrop. |
...
| **Structure** | Current full-page analysis | Exposes headings, semantic review and structural metrics prepared with the page scan; Refresh recalculates them after page changes. Native headings that redundantly repeat role="heading"/aria-level are counted once, and links whose title merely repeats their visible text are surfaced as review guidance. | H1-H6 outline, suggestions and counts; headings without direct text can use meaningful accessible descendant content, while decorative-only content remains empty. |
| **Trace** | Real interaction | Records keyboard/pointer input, focus, non-sensitive setting-change events, relevant mutations, SPA routes, dialogs, status-message candidates, ARIA widgets and causal/context-change evidence. Starting/resuming Trace or Focus Walk checks the selected tab against the active audit sites before recording. A full document navigation pauses Trace until the new page is validated. | Events correlated by interaction and attributed to a redacted page URL; control values are not retained by context-change tracking. |
...
| **Report** | Available static/runtime evidence | Consolidates analysis, runtime stories and already-generated Structure data. Multipage audits retain up to 200 recent page-attributed Trace events per reviewed page. | Report view and exports with bounded local evidence. |
| **PDF / TXT / Markdown / JSON** | Current report or Memory baseline | Exports the applicable evidence and auditor notes without silently rerunning a full DOM collection. Complete audit PDFs place retained Trace evidence inside its matching page section; individual page/component PDFs include a compact linked section index, using the multipage index's regular typography, dotted leaders and spacing without solid row separators. | Shareable artifacts. |

Before Trace/Focus Walk starts (including resume after a breakpoint or document navigation), another site requires an explicit choice: cancel, add the site to the current audit, or start a separate audit. Pages on an already accepted site do not prompt. A full navigation while recording pauses Trace, preserves the previous evidence and records the redacted origin/destination transition; recording resumes only after the current page passes the same scope check. Cancelling preserves the previous Trace; stopping a recording never prompts. Navigation while choosing invalidates the decision. Choosing a scope saves audit membership only, not a fabricated scan or historical Trace snapshot. The responsive scope dialog expands up to 720px, keeping 12px side gutters in narrow panels.

When an analyzed page belongs to the active multipage audit, attributed Trace evidence is persisted with that page on stop, navigation pause, breakpoint pause or page reanalysis. Repeated synchronization deduplicates events by ID, and reanalyzing the same URL updates its existing report without moving it or discarding its saved Trace. If redacted URLs cannot identify a unique reviewed page, FocusTrace does not guess the association.

The current audit heading and scope dialog list all accepted domains, wrapping on narrow panels; the initial audit name no longer hides a multisite scope.

**Report → More formats → Export JUnit (.xml)** exports current or saved page/component findings using the same JUnit contract as CLI/Playwright: only FAIL creates a failing testcase. Historical reports export their static findings without borrowing live Trace events. The Markdown/JSON buttons export Trace evidence, not static audits; when unavailable, the menu explains the missing recording.

Repeated static findings are deduplicated only when **rule, target and evidence are all the same**. Evidence changes on the same rule/target remain visible and become `changed`; an absent prior identity becomes `resolved` only in the comparison history, and a later reappearance becomes `new` again. Lifecycle comparison is limited to compatible document/scope/profile contexts. Recheck keeps its additional `missing`/`inconclusive` safety states, while Replay and Site Audit use the same `new`/`persistent`/`changed`/`resolved` vocabulary where a complete comparison is available. Exact evidence matches are reserved before changed findings are paired, so result ordering cannot turn an unchanged finding into a change.
...
Audit profiles are stored only in the browser profile and are bounded to one built-in complete profile plus **20 custom profiles**. The built-in profile preserves all supported findings; custom profiles can select page/component/Site Audit scope, all supported rules or a WCAG A/AA/AAA target, standards sources, exact FocusTrace rule IDs, severities and rule families. Profiles filter stored/reporting findings; they do not disable the underlying local rule engine, alter PASS meaning, transmit page data or change the inspected page.
...
`FT-WCAG-014`, `FT-WCAG-015` and `FT-WARN-022` reuse the same bounded accessible-name evidence model. Tabs, tooltips, treeitems and native `summary` controls can use name-from-content where their semantics allow it; a treeitem excludes descendant `group` content from its own computed content name. Dialogs, meters and progress indicators rely on author-provided naming mechanisms. Programmatically hidden targets are inapplicable, and broken `aria-labelledby` references remain visible in diagnostic candidates instead of being silently repaired.
...
| `FT-REVIEW-024` | At an effective 320 CSS px viewport (or 256 CSS px height for vertical writing), non-exempt content causes cross-axis document scrolling or rendered text/controls are clipped by `overflow: hidden/clip`. | REVIEW / PASS | WCAG 1.4.10 AA |
| `FT-REVIEW-025` | A rendered native link inside prose differs from adjacent non-link text by color, has less than a 3:1 lightness difference and exposes no persistent underline, typographic, boundary or graphic cue that FocusTrace can resolve. | REVIEW / PASS | WCAG 1.4.1 A |
...
| `FT-REVIEW-040` | Multiple native labels, title-only control naming, unnamed control groups, constraint metadata without a resolved instruction relationship, or visible required wording without programmatic required state. | REVIEW | WCAG 1.3.1 / 3.3.2 · HTML forms |
...
`FT-REVIEW-024` inspects the page's **current** responsive state; it never changes browser zoom or viewport size. Run Analyze after zooming or narrowing the page until its effective viewport is at most 320 CSS px wide for horizontal writing (or 256 CSS px high for vertical writing). FocusTrace measures document overflow, identifies bounded non-exempt protruding targets, and detects observable text/controls clipped by an unscrollable `overflow: hidden/clip` ancestor. Known two-dimensional surfaces such as data tables, SVG/canvas graphics, video and embedded applications are excluded from automatic review. The result remains REVIEW because essential-layout exceptions, responsive replacement content, overlays, Shadow DOM and visual meaning still require manual inspection.

`FT-REVIEW-025` provides a conservative automated candidate search for the inline-link subset of WCAG 1.4.1. It evaluates up to 2,000 rendered native links only inside prose-like `p`, `li`, `dd`, `dt`, `figcaption` and `blockquote` contexts, excluding navigation/menu/toolbar regions and standalone links, and emits at most 50 reviews per scan. A link is reviewed only when its rendered text color differs from comparable adjacent non-link text by less than `3:1` and no persistent non-color cue is observable in the normal state. Underlines, typographic differences, visible boundaries, generated content and rendered graphics suppress the review; equal colors, different/complex backgrounds, hidden content and unresolved rendering are omitted. Structured JSON evidence preserves both colors, the measured ratio and context selectors. The result remains REVIEW because visual context and uncommon author cues still require human confirmation, and other use-of-color cases such as errors, required fields, charts and state indicators remain manual.

`FT-REVIEW-026` provides a conservative current-state candidate search for the moving, blinking and scrolling branch of WCAG 2.2.2. It inspects up to 1,000 active browser-exposed Web Animations, 250 rendered `<marquee>` elements and 250 rendered native `video[autoplay]` elements, only when other visible content is observable, and emits at most 50 reviews per scan. Web Animations with a resolved total active duration of five seconds or less and autoplay video of no more than five seconds record bounded PASS; native video controls also record bounded PASS. A rendered, accessibly named control explicitly related through `aria-controls` is preserved as a candidate but does not suppress REVIEW because its behavior must be exercised. Structured JSON evidence records source, duration or indefinite repetition, changed keyframe properties, automatic-start confidence and control selectors. The result remains REVIEW because essentiality, actual automatic start and custom-control behavior require human verification. Timed DOM/text updates, animated image pixels, canvas/SMIL/custom players, Shadow DOM and cross-origin frames remain manual.

`FT-REVIEW-027` complements the empty-name failure in `FT-WCAG-005`: it reviews only exposed semantic links whose non-empty accessible name exactly matches a deliberately small, accent-insensitive list of generic English or Spanish phrases such as “read more”, “click here”, “details”, “leer más” or “aquí”. It inspects at most 2,000 link candidates and emits at most 50 reviews per page or component scan. Structured JSON evidence preserves bounded text and selectors for resolved `aria-describedby` targets, the containing sentence or paragraph, current and parent list items, table cells and table headers associated explicitly or through `scope`, and the closest block container. Context never changes the outcome to PASS because deciding whether natural-language wording identifies the link purpose requires human judgement; a visually preceding heading alone is not treated as normative programmatic context. Custom phrase variants, semantics in Shadow DOM or cross-origin frames, language understanding and the WCAG generally-ambiguous exception remain manual.

`FT-REVIEW-028` is a guided full-page workflow for WCAG 1.4.4. Analyze at 100% browser zoom to retain a session-only reference of up to 1,000 rendered direct-text and interactive-control subjects, then set browser zoom to 200% and analyze the same document again. FocusTrace compares stable selectors and bounded text/name signatures, suppresses an unavailable original when an equivalent rendered replacement is observable, and emits at most 30 reviews for newly unavailable content, lost control names, new clipping, new overlap or effective text scaling below 1.9:1. Structured JSON evidence preserves both zoom factors, baseline/current rectangles, scale and related selectors. FocusTrace never changes zoom. Intermediate zoom steps, semantic equivalence, user-agent/platform UI, images of text, captions, transforms, pseudo-generated/Shadow DOM text, cross-origin frames and complete functionality remain manual; a quiet comparison is not proof of WCAG 1.4.4 conformance.

For `FT-REVIEW-019`, FocusTrace applies only when it observes explicit `aria-invalid` (except `false`) or browser-supported `:user-invalid`. A non-empty `aria-errormessage` or `aria-describedby` target is bounded PASS evidence for association presence only; FocusTrace does not prove that the text completely describes the error or that no visible/application-level message exists elsewhere. `FT-REVIEW-020` runs only when that invalid field already has associated text and exposes a correction-relevant constraint such as `required`, a constrained input type, `pattern`, `min`/`max`, `step` or length bounds. It remains REVIEW because suggestion adequacy and the WCAG security/purpose exception require context. Neither rule reads or stores the field value, and associated error-message text is used only to establish that non-empty text exists; the message itself is not persisted in finding evidence.

For `FT-REVIEW-040`, FocusTrace groups bounded form-authoring signals that need human context: more than one native `<label>` associated with a control, a control whose accessible name comes only from `title`, an exposed `fieldset`/`group`/`radiogroup` with multiple controls but no usable group name, constraint metadata without a resolved non-empty `aria-describedby`/`aria-details` relationship, and visible required wording without native `required` or `aria-required="true"`. These remain **REVIEW** because multiple labels and instruction placement can be intentional and because semantic adequacy cannot be inferred from structure alone. The rule never reads or stores editable field values.
...
| Form errors | Static review observes explicit/user-invalid state, ARIA-associated non-empty error text presence and correction-relevant constraint metadata only. It does not read field values, persist associated message text, infer every visual/application-level message or determine semantic adequacy of a correction suggestion. |
| Form labels and instructions | `FT-REVIEW-040` inspects bounded structural signals only: multiple native labels, title-only naming, unnamed groups, constraint metadata without resolved instruction relationships and visible required wording without programmatic required state. It does not judge instruction quality automatically or persist editable values. |
...
| Input purpose | Only explicit standard-like `autocomplete` token sequences are validated. Missing `autocomplete`, unknown-only custom taxonomies and whether a field actually collects information about the user remain outside automatic judgement. Readonly controls are excluded from this review family. |
...
Production builds do not require global host access at install time. HTTP/HTTPS access is optional and requested from explicit user actions. Permission-sensitive page tools start that request directly from the native click before asynchronous UI work, preserving Firefox's user-action requirement, and confirm Firefox's retained grant when a repeated request returns without another dialog. Firefox 128+ exposes these runtime grants through the Manifest V3 `optional_host_permissions` field, while the experimental Firefox 115-127 target retains its compatible legacy declaration. Broad `<all_urls>` capture access is requested only when a visual export needs it and is removed after the operation when FocusTrace acquired it for that export.
...
### Frames, SVG, image maps and embedded content

FocusTrace extends the shared accessible-name model to embedded-content checks without creating parallel findings for cases already covered by an existing rule. `FT-WCAG-018` checks exposed `object[data]` elements for a non-empty accessible alternative or explicit presentational treatment. `FT-WCAG-019` checks exposed `iframe` / `frame` elements for a non-empty accessible name. `FT-WCAG-020` checks a frame removed from sequential navigation with negative `tabindex` when its same-origin embedded document is locally inspectable; an observed sequentially focusable descendant is a deterministic FAIL for that bounded keyboard expectation.

`FT-REVIEW-038` keeps duplicate normalized frame names as REVIEW because identical names are valid when the embedded contexts genuinely serve an equivalent purpose. `FT-REVIEW-039` makes the inspection boundary explicit when a nested document cannot be evaluated because it is cross-origin, sandboxed without same-origin access, opaque or otherwise unavailable. Settings also provides an **Ignore iframe contents** preference, disabled by default. When enabled, FocusTrace keeps evaluating the exposed `<iframe>` / `<frame>` element itself but intentionally does not traverse or audit descendants inside its embedded document, including same-origin content.

SVG and image-role alternatives continue to use `FT-WCAG-002`, including direct SVG `<title>` text where supported; active image-map areas continue to use the existing link-name rule `FT-WCAG-005`. This avoids duplicate findings while keeping the same accessible-name evidence primitive. Nested frame inspection persists only the evaluation boundary and, when relevant, the tag name of an observed focusable descendant; embedded document text and attributes are not copied into findings, Memory or reports.

### Form labels, purpose, constraints and errors
...
Backdrop verification remains bounded to 100 contrast candidates per scan and 800 computed-style observations shared across that verification pass. It first samples the actual paint stack at the target center and four interior points, then uses a much smaller bounded structural fallback (up to 8 ancestors, 12 siblings and 24 descendants per branch). It reuses computed styles/geometry and never performs a stylesheet-wide selector/rule scan for every candidate. If either budget is exhausted, remaining candidates are explicitly REVIEW because their backdrop was not verified, with an exhaustion reason and consistent counters. Opaque RGB colors (including black and red) are distinguished from transparent RGBA colors.
...
The guided framework now includes keyboard operability, focus order/visibility and dialog-lifecycle workflows. These tests reuse the existing FocusTrace runtime event stream: relevant Trace events are attached as bounded `runtime-observation` evidence when a step is saved, while the auditor's answer remains a separate manual record.
...
Report groups guided tests in an initially collapsed **Manual checks** accordion. Review focuses on findings and no longer displays the lifecycle summary or the zoom-status card; underlying lifecycle data and resize analysis remain available.
...
The 1.0.9 session/persistence review also serializes multipage audit mutations through a single background writer, preventing simultaneous panels from overwriting saved pages. Clears wait for earlier accepted writes, and closing a tab removes its session after pending writes finish. The panel rejects obsolete session/audit refreshes and previous-tab action responses; changing language no longer restarts tab selection. Pending scope decisions are cancelled when their tab changes.
