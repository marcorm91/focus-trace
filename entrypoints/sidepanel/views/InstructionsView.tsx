    ...
    <section className="panel instructions-panel" aria-labelledby="instructions-title">
      ...
      <div className="instructions-grid">
        ...
        <InstructionCard title={tr(language, 'DevTools and DOM inspection', 'DevTools e inspección del DOM')}>
          <p>{tr(language, 'In Chrome, Edge and Firefox, FocusTrace can run as a dedicated panel inside browser DevTools. Open DevTools with F12 and select the FocusTrace tab to use the developer-integrated workspace.', 'En Chrome, Edge y Firefox, FocusTrace puede ejecutarse como panel dedicado dentro de las DevTools del navegador. Abre DevTools con F12 y selecciona la pestaña FocusTrace para usar el espacio de trabajo integrado para desarrollo.')}</p>
          <ul>
            ...
            <li>{tr(language, 'Inspect in DOM is available when FocusTrace is opened inside browser DevTools. It selects the exact node matched by the finding selector in Elements on Chromium or the Inspector on Firefox.', 'Inspeccionar en el DOM está disponible cuando FocusTrace se abre dentro de las DevTools del navegador. Selecciona el nodo exacto que coincide con el selector del hallazgo en Elements de Chromium o en el Inspector de Firefox.')}</li>
          ...
          <p>{tr(language, 'Firefox keeps the normal FocusTrace sidebar too. The Firefox DevTools permission is optional: enable DevTools integration once from Settings, then open or reopen F12 → FocusTrace to use native DOM inspection.', 'Firefox también mantiene el sidebar normal de FocusTrace. El permiso de DevTools de Firefox es opcional: activa una vez la integración DevTools desde Ajustes y después abre o vuelve a abrir F12 → FocusTrace para usar la inspección nativa del DOM.')}</p>
        </InstructionCard>
        ...
        <InstructionCard title={tr(language, 'Settings and privacy', 'Ajustes y privacidad')}>
          <p>{tr(language, 'Settings lets you change language, interface size, runtime breakpoints, Memory preferences and, in Firefox, optionally enable the FocusTrace DevTools panel.', 'Ajustes permite cambiar el idioma, el tamaño de interfaz, los breakpoints runtime, las preferencias de Memory y, en Firefox, activar opcionalmente el panel FocusTrace de DevTools.')}</p>
