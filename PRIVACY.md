...
After optional HTTP/HTTPS access is granted, FocusTrace may register a persistent, local MAIN-world ElementInternals bridge at document start on permitted pages. The bridge observes subsequently created custom-element semantics so an explicit later analysis can inspect otherwise unavailable evidence. Registering this bridge does not start a full accessibility scan or a Trace, and does not upload inspected-page data. Browsers retain the optional grant until it is revoked in extension site-access settings; unavailable bridge evidence is treated conservatively.
