...
const NEW_KEYBOARD_RULES = [
  ['FT-APG-015', 'Tab arrow navigation did not reach the expected tab', 'pestañas'],
  ['FT-APG-016', 'Radio group arrow navigation did not reach the expected radio', 'grupo de opciones'],
  ['FT-APG-017', 'Toolbar arrow navigation did not reach the expected control', 'barra de herramientas'],
  ['FT-APG-018', 'Menu arrow navigation did not reach the expected item', 'menú'],
  ['FT-APG-019', 'Listbox arrow navigation did not reach the expected option', 'listbox'],
  ['FT-APG-020', 'Escape did not close the open modal dialog', 'diálogo modal'],
  ['FT-APG-021', 'Disclosure keyboard activation did not toggle the expanded state', 'activación por teclado'],
] as const;

describe('keyboard focus bilingual presentation', () => {
  it.each(NEW_KEYBOARD_RULES)('localizes %s without changing technical evidence', (ruleId, title, spanishFragment) => {
    ...
    expect(humanRuntimeEventTitle(event, 'en')).toBe(title);
