...
export function buildPageInspectorEntries(
  ...
  return focusPath.map((target) => {
    ...
    const seriousRuntime = runtimeFindings.some((event) =>
      ['critical', 'serious'].includes(event.severity),
    );

    let tone: FocusPathOverlayTone = 'ok';
    if (missingName || positiveTabindex || failedScan || seriousRuntime) tone = 'fail';
    else if (scanFindings.length || runtimeFindings.length || repeated) tone = 'review';

    const primaryScan = scanFindings.find((issue) => issue.outcome === 'fail') ?? scanFindings[0];
    const primaryRuntime = runtimeFindings.find((event) =>
      ['critical', 'serious'].includes(event.severity),
    ) ?? runtimeFindings[0];
    ...
    const status = tone === 'fail'
      ? translated(language, 'Likely failure', 'Fallo probable')
      : tone === 'review'
        ? translated(language, 'Review', 'Revisar')
        : translated(language, 'No signals', 'Sin señales');
