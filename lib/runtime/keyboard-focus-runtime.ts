...
function managedRoleItems(group: Element, roles: string[]): Element[] {
  return ownedRoleElements(group, roles)
    .filter((candidate) => candidate !== group && belongsToManagedGroup(group, candidate) && isAvailable(candidate));
}
...
function tabNavigation(target: Element, key: string): KeyboardFocusProbe | undefined {
...
function radioNavigation(target: Element, key: string): KeyboardFocusProbe | undefined {
...
function menuNavigation(target: Element, key: string): KeyboardFocusProbe | undefined {
...
function listboxNavigation(target: Element, key: string): KeyboardFocusProbe | undefined {
...
function rovingProbeForTarget(target: Element): KeyboardFocusProbe | undefined {
  const group = target.closest(MANAGED_GROUP_SELECTOR);
  if (!group) return undefined;
  const role = semanticRole(group) as ManagedGroupRole | undefined;
  ...
  if (role === 'radiogroup' && !group.closest('[role="toolbar"]')) {
    return { kind: 'managed-roving-tabindex', group, roles: ['radio'], pattern: 'radio group' };
  }
...
export function captureKeyboardFocusProbes(
  ...
  const probes: KeyboardFocusProbe[] = [];
...
export function evaluateKeyboardFocusProbe(probe: KeyboardFocusProbe): PendingRuntimeEvent | undefined {
  switch (probe.kind) {
